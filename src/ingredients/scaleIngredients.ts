...
export function scaleIngredientsByIngredient(
  ...
  if (oldIngredient) {
    const ratio = calculateRatioBetween(
      ingredient.measurement,
      oldIngredient.measurement
    );
...
function calculateRatioBetween(
  first: Measurement,
  second: Measurement
): number {
...
function scaleIngredient(
  factor: number
): (ingredient: MeasuredIngredient) => MeasuredIngredient {
  return ingredient => ({
    ...ingredient,
    measurement: scale(ingredient.measurement, factor)
  });
}
