...
import { grams, Gram, Milligram, Ounce, Kilogram, Pound, Litre, Millilitre, Pint, Fahrenheit, Celsius, Teaspoon, Tablespoon, FluidOunce, Cup, Gallon, UnitMap } from '../measurement/units';
...
const unitMap = allUnits.reduce((map, measurement) => {
...
export function parseIngredient(original: string): ParsedIngredient {
  ...
  return { original, ...buildMeasuredIngredient(matches[4].trim(), measurement) };
}
